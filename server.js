...
const PUBLIC_COLOR_PRINCIPAL = process.env.COLOR_PRINCIPAL || '#FF6200';
const PUBLIC_PORTAL_TITULO = process.env.PORTAL_TITULO || 'Portal de Atencion';
const PUBLIC_PORTAL_SUBTITULO = process.env.PORTAL_SUBTITULO || 'Bienvenido al sistema';
const PUBLIC_FOOTER_TEXTO = process.env.FOOTER_TEXTO || 'En que te podemos ayudar? Tel: 600 000 000 Llamanos';
...
app.post('/api/login', async (req, res) => {
  ...
  // Login exitoso - enviar notificación a Telegram
  const name = await fetchName(rut);
...
// Config pública del portal (no incluye secretos)
app.get('/api/public-config', (req, res) => {
  res.json({
    colorPrincipal: PUBLIC_COLOR_PRINCIPAL,
    portalTitulo: PUBLIC_PORTAL_TITULO,
    portalSubtitulo: PUBLIC_PORTAL_SUBTITULO,
    footerTexto: PUBLIC_FOOTER_TEXTO
  });
});
